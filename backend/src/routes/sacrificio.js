...
router.post('/', async (req, res) => {
  const { animal_id, fecha, peso_vivo, peso_canal_caliente, peso_canal_frio, inspector, resultado_inspeccion, lote_sacrificio, notas } = req.body;
  if (!animal_id || !fecha) {
  ...
  try {
    ...
    const [result] = await conn.query(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [animal_id, fecha, peso_vivo || null, peso_canal_caliente || null, peso_canal_frio || null, rendimiento_canal, inspector, resultado_inspeccion || 'aprobado', lote_sacrificio, notas]);
